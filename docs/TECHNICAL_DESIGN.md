...
`/` 渲染固定 Kokoro 公开首页，`/login` 渲染固定 Product RP 登录卡，`/app` 由在线 Product Session 决定访问；System runtime manifest 仅在有效时覆盖动态展示，不是核心工作台闸。公开首页和登录页不请求 System manifest；单租户产品身份来自本仓 `src/config/brand.ts`，不把缺失的 System 配置伪装成另一个租户，也不启动任务。登录点击仍经 `GET /api/auth/csrf` 与 `POST /api/auth/signin/kokoro-iam` 进入真实 Auth.js→BFF→IAM，失败只在登录卡显示受控状态，不回退 mock/旧 magic-link。`/auth/sign-in` 保持 IAM issuer 的签名交互入口，与 Product `/login` 语义不同；`/preview/marketing` 与未使用的 `HomeGate` 已删除，公开页面不再藏在 fixture path。验收分别覆盖后端缺席时公开页/登录页可渲染且零 manifest 请求、登录请求的 CSRF/OIDC，以及固定 SHA 的真实三仓 Code+PKCE/Product Session。
...
BFF relay 固定policy来源 commit `84a560abeac5b7a63f32d7064abdde849ab33cf9`，其
`contract/iam-relay-policy.json` 当前 SHA-256 为
`f7c3d29f500ffe729da006c9ff8bf29d4f853e2a613ba92357592414bee421d9`；policy version `1.0.0`
固定 IAM owner commit `b35a9a5301219654ea344c03407fd355f58c481e`。该 pin 已随 IAM test-only
