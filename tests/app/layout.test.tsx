...
function isBodyElement(value: ReactNode): value is ReactElement<{ children?: ReactNode }> {
  return isValidElement(value) && value.type === "body"
}
...
describe("RootLayout language", () => {
  ...
  it("wraps app content with the theme and locale providers", async () => {
    const { default: RootLayout } = await import("@/app/layout")

    const element = await RootLayout({ children: <span>child</span> })
    // <html> 下现有 <head>（首帧主题脚本）+ <body> 两个子节点；从中取 <body>。
    const body = Children.toArray(element.props.children).find(isBodyElement)
    if (body === undefined) {
