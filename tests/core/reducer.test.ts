...
describe("event_id 幂等去重", () => {
  it("同一 event_id 第二次折叠原样返回同一引用", () => {
    ...
    const once = applyChatProjectionEvent(createSessionStreamState(), event)
    const twice = applyChatProjectionEvent(once, event)
  ...
  it("批内重复 event_id 只折叠一次", () => {
    const event = makeEvent("message.delta", { segment_id: "seg_1", delta: "hi" })
    const state = applyChatProjectionEvents(createSessionStreamState(), [event, event, event])
...
describe("replay 收敛", () => {
  ...
  it("整批折叠与逐事件折叠等价", () => {
    ...
    const batched = applyChatProjectionEvents(createSessionStreamState(), events)
    const oneByOne = events.reduce(applyChatProjectionEvent, createSessionStreamState())
    expect(batched).toEqual(oneByOne)
  })

  it("重复 replay 同一批事件收敛到相同状态", () => {
    const events = fullRun()
    const once = applyChatProjectionEvents(createSessionStreamState(), events)
    const replayed = applyChatProjectionEvents(once, events)
    expect(replayed).toBe(once)
  })

  it("message.completed 覆盖累计增量，replay 不残留半句", () => {
    const state = applyChatProjectionEvents(createSessionStreamState(), fullRun())
  ...
  it("snapshot 水合置位、匹配终态清空", () => {
    let state = stateFromSnapshot(
      makeSnapshot({
        ...
        eventWatermark: "agui_00000000000000000000000000000003",
      }),
    )
...
describe("message.user 与本地 echo 对齐（SSE/receipt 竞态）", () => {
  ...
  it("receipt 先对齐过 id：事件按 id 命中更新，不新建", () => {
    ...
    state = applyChatProjectionEvent(state, makeEvent("message.user", { message_id: "msg_u1", content: "hi" }))
    expect(state.messages).toHaveLength(1)
  })
...
describe("Schema 崩溃矩阵（契约入站防线）", () => {
  it.each([
  ...
  ])("%s 被 parseChatProjectionEvent 拒绝", (_label, overrides) => {
    const base = {
      kind: "message.delta",
      payload: { segment_id: "s", delta: "x" },
      event_id: "e1",
    ...
    expect(() => parseChatProjectionEvent({ ...base, ...overrides })).toThrow()
  })
})
