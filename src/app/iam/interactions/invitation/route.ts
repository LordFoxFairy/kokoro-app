...
import { isExpiredInvitationError, parseInvitationContext, parseInvitationTarget, readInvitationForm, VERIFY_MESSAGES } from "@/lib/server/iam-invitation-input"
...
function rejected(status: number, id: string): Response {
  ...
  return page(invitationStatusPage(message, status === 404 ? "当前账号无法查看这份邀请。" :
    status === 429 ? "请稍后再试。" :
      status === 403 ? "请检查邀请链接。" : "请稍后通过原邀请链接继续。"), status, id)
}
...
export async function GET(request: Request): Promise<Response> {
  ...
  try {
    ...
    return page(invitationPreviewPage({ tenantName: context.tenant_name,
      roles: context.roles, expiresAt: context.expires_at }), 200, id)
...
export async function POST(request: Request): Promise<Response> {
  ...
  const action = form.get("decision") as "sign-in" | "sign-up"
  const cookieName = action === "sign-in" ? "kokoro_iam_csrf_invite_signin" : "kokoro_iam_csrf_invite_signup"
  try {
    const accepted = await consumeIamInteractionCsrf({ redisUrl, webOrigin: config.webOrigin, path: PAGE_PATH,
      method: "POST", query: target.query, issuerCookie: "", context: action,
    ...
    if (upstream.status !== 200) {
      const failure = upstream.status === 429 ? "尝试次数过多，请稍后再试。" :
        upstream.status === 400 || upstream.status === 401
          ...
          : "本次操作未完成，请稍后再试。"
      return loginPage({ query: target.query, requestId: id, redisUrl, webOrigin: config.webOrigin,
        secureCookies: config.secureCookies, message: failure })
    }
    if (!/^application\/json(?:; charset=utf-8)?$/iu.test(upstream.headers.get("content-type") ?? "") ||
