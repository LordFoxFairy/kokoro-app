...
.sign-in-page .brand-panel { width: min(100%, 26rem); margin: 0; padding: 0; text-align: center; }
.sign-in-page .interaction { display: block; flex: none; width: min(100%, 26rem); padding: 0; }
.sign-in-page .content { width: 100%; padding: clamp(1.5rem, 5vw, 2rem); border: 1px solid var(--border); border-radius: calc(var(--radius) * 1.25); background: var(--card); box-shadow: 0 8px 28px -18px color-mix(in srgb, var(--foreground) 20%, transparent); }
.sign-in-page h1 { font-size: clamp(1.6rem, 4vw, 1.85rem); line-height: 1.3; }
.sign-in-page .lead { margin-top: .5rem; font-size: .925rem; }
...
@media (max-width: 720px) { .sign-in-page { align-content: start; gap: 1.5rem; padding: 2rem 1rem; } }
`
...
export function iamInteractionDocument(input: Readonly<{
  ...
  const lang = input.variant === "compact-invitation" || input.variant === "sign-in" ? "zh-CN" : "en"
