...
export function invitationSignInPage(input: Readonly<{
  ...
  const forms = `${notice}<form class="auth-form" method="post" action="${action}">
    <input type="hidden" name="csrf_token" value="${escapeHtml(input.signInToken)}">
    <input type="hidden" name="decision" value="sign-in">
    <label class="field" for="invite-email">邮箱<input id="invite-email" name="email" type="email" autocomplete="username" required></label>
  ...
  </form><details class="invitation-register"><summary>没有账号？创建 Kokoro 账号</summary>
    <form class="auth-form" method="post" action="${action}">
      ...
      <label class="field" for="invite-name">姓名<input id="invite-name" name="name" type="text" autocomplete="name" required></label>
      <label class="field" for="invite-register-email">邮箱<input id="invite-register-email" name="email" type="email" autocomplete="email" required></label>
