...
## 职责
会话态探针 + 首页分流 + 登录页面板（AUTH-P0 / WEB-FACE 面一·面二）。鉴权由 HttpOnly Product Session cookie 同源携带，前端不持 token。
...
- `LoginPanel`（`login-panel.tsx`）：`/login` 登录卡（暖纸皮肤，无 antd）。以 Auth.js CSRF 启动固定 `kokoro-iam` OIDC；错误走顶部 toast（不内联）；不渲染未接入的 OAuth 假按钮；品牌/Logo/主题由 System runtime manifest 注入。
