...
- `LoginPanel`（`login-panel.tsx`）：`/login` 登录卡（暖纸皮肤，无 antd）。独立于 System manifest 使用固定单租户 Kokoro 品牌；以 Auth.js CSRF 启动固定 `kokoro-iam` OIDC，只有登录操作故障时显示受控 toast；不渲染未接入的 OAuth 假按钮。

## 协作者
上游：`@/app/page.tsx`（公开首页）、`@/app/login/page.tsx`（LoginPanel）、`@/ui/settings`（useSessionState 匿名闸）。下游：`@/api/auth/*` BFF、`@/i18n`（auth.* 文案）、`@/ui/marketing`、`@/ui/shell`。只有 `AppGate` 消费 System runtime manifest。
